...
function new_markup($video_id) {
    return '<div id="mainplayer" class="ndn_embed" data-config-widget-id="2" data-config-type="VideoPlayer/Single" data-config-tracking-group="90115" data-config-playlist-id="18497" data-config-video-id="' . $video_id . '" data-config-site-section="denverpost" data-config-height="9/16w"></div><script src="' . get_template_directory_uri() . '"/scripts/inform.js?"></script>';
}

function run_replace( $csv, $test, $test_rows ) {
    if ($test) {
        $limit = ( $test_rows && $test_rows > 0 ) ? (int)$test_rows : false;
        $result = ( $limit ) ? '<p><strong>Test limited to ' . $limit . ' rows.</strong></p>' : '';
    }
    $i=0;
    $lines = array();
    foreach (preg_split('/\r\n?|\n/', $csv) as $line) {
        ...
        if (!$limit || $i < $limit) {
            ...
            if (!empty($post_ids_videos[0])) {
                foreach ($post_ids_videos as $post_id_video) {
                    ...
                    if (!$test) {
                        update_post_meta($post_id_video,'video_id',$line[0]);
                        update_post_meta($post_id_video,'main_video',$new_video_markup);
                    }
                }
            } else {
